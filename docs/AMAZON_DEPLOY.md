...
8. [문제 해결](#8-문제-해결)
9. [보안 권장사항](#9-보안-권장사항)
10. [프리티어 주의사항](#10-프리티어-주의사항)
11. [24시간 백그라운드 실행](#11-24시간-백그라운드-실행)
...
TUI(Terminal User Interface)는 터미널에서 트레이딩 시스템을 제어하는 인터페이스입니다.

### 6.1 TUI 실행

```bash
# Docker로 TUI 실행 (대화형 모드)
docker compose run --rm --profile tui tui

# 또는 직접 실행
docker compose run --rm -it app python scripts/run_tui.py
```

> **중요**: TUI는 대화형 터미널이 필요합니다. SSH 세션에서 실행하세요.
...
```
┌─────────────────────────────────────────────────────────────┐
│ 터틀-캔슬림                           CANSLIM + 터틀 트레이딩 │  ← 헤더
├─────────────────────────────────────────────────────────────┤
│ 모드: PAPER  포지션: 0  유닛: 0/20  후보종목: 5  최근스캔: --  │  ← 상태바
├─────────────────────────────────────────────────────────────┤
│ [포트폴리오] [후보종목] [시그널] [설정] [단축키]               │  ← 탭
├─────────────────────────────────────────────────────────────┤
│                                                             │
│                    (탭 내용 영역)                            │
│                                                             │
├─────────────────────────────────────────────────────────────┤
│ [새로고침] [KRX스크리닝] [US스크리닝] [전체스크리닝]           │  ← 버튼
│           [KRX트레이딩] [US트레이딩]                         │
├─────────────────────────────────────────────────────────────┤
│ 10:30:15 데이터 새로고침 완료                                │  ← 로그 패널
...
|----|------|------|
| **Q** | 종료 | TUI 종료 |
| **R** | 새로고침 | DB에서 데이터 다시 읽기 |
| **U** | 데이터 갱신 | 오래된 가격 데이터 업데이트 |
| **S** | 전체 스크리닝 | KRX + US CANSLIM 스크리닝 |
| **K** | KRX 스크리닝 | 국내 주식만 스크리닝 |
| **N** | US 스크리닝 | 미국 주식만 스크리닝 |
| **T** | KRX 트레이딩 | KRX 트레이딩 시작/중지 토글 |
| **Y** | US 트레이딩 | US 트레이딩 시작/중지 토글 |
| **D** | 다크모드 | 다크/라이트 모드 전환 |
| **1-5** | 탭 전환 | 1=포트폴리오, 2=후보종목, 3=시그널, 4=설정, 5=단축키 |

### 6.4 탭 설명

| 탭 | 내용 |
|----|------|
| **포트폴리오** | 현재 보유 포지션 (종목, 수량, 매입가, 현재가, 손익, 손절가) |
| **후보종목** | CANSLIM 스크리닝 결과 (점수, C/A/N/S/L/I/M 통과 여부, RS, EPS%) |
| **시그널** | 최근 트레이딩 시그널 (진입/청산/피라미딩) |
| **설정** | 현재 설정값 확인 (CANSLIM 기준, 터틀 설정, 리스크 관리) |
| **단축키** | 키보드 단축키 안내 |
...
```bash
# TUI에서 트레이딩 시작
# T 키: KRX 트레이딩 시작 (다시 누르면 중지)
# Y 키: US 트레이딩 시작 (다시 누르면 중지)

# 트레이딩 중:
# - 버튼이 "KRX중지 [T]"로 변경됨
# - 로그 패널에 실시간 시그널 및 체결 정보 표시
# - 시장 마감 시 자동으로 대기, 개장 시 재개
```

**트레이딩 모드 확인**:
- 상태바에 `PAPER` 또는 `LIVE` 표시
- PAPER: 모의투자 (안전)
- LIVE: 실거래 (주의!)

### 6.7 터미널 권장사항

한글이 깨져 보이면 터미널을 변경하세요:
...
| WezTerm | ✅ | 크로스플랫폼 |
| Kitty | ✅ | 빠름 |
| GNOME Terminal | ✅ | Linux 기본 |
| Apple Terminal | ⚠️ | 일부 깨짐 |
| Windows Terminal | ✅ | Windows 권장 |
...
# 10. TUI 실행 (선택)
docker compose run --rm --profile tui tui
# TUI에서: K=KRX스크리닝, N=US스크리닝, T=트레이딩 시작
```
...
# ===== TUI (터미널 UI) =====
docker compose run --rm --profile tui tui    # TUI 실행 (대화형)

# TUI 단축키:
#   Q=종료  R=새로고침  U=데이터갱신
#   S=전체스크리닝  K=KRX스크리닝  N=US스크리닝
#   T=KRX트레이딩  Y=US트레이딩  D=다크모드
#   1-5=탭전환
...
터미널을 종료해도 트레이딩 봇이 계속 실행되도록 설정하는 방법입니다.

### 11.1 Docker Compose (기본 - 이미 지원됨)

`docker compose up -d`의 `-d` 플래그가 백그라운드 실행을 의미합니다.

```bash
# 백그라운드 실행 (터미널 종료해도 유지됨)
docker compose up -d
...
Docker 컨테이너는 **서버 재부팅 시에도 자동 시작**됩니다 (`restart: unless-stopped` 설정).

### 11.2 tmux (Docker 없이 직접 실행 시)

Docker 없이 Python으로 직접 실행할 경우:
...
### 11.3 systemd 서비스 (Docker 없이 프로덕션 권장)

서버 재부팅 시 자동 시작이 필요하면 systemd 서비스로 등록:

```bash
sudo nano /etc/systemd/system/turtle-trading.service
```

```ini
[Unit]
Description=Turtle CANSLIM Trading Bot
After=network.target

[Service]
Type=simple
User=ubuntu
WorkingDirectory=/home/ubuntu/apps/turtle-canslim
ExecStart=/home/ubuntu/apps/turtle-canslim/.venv/bin/python scripts/run_trading.py
Restart=always
RestartSec=10
...
sudo systemctl enable turtle-trading   # 부팅 시 자동 시작
sudo systemctl start turtle-trading    # 즉시 시작

# 관리 명령어
sudo systemctl status turtle-trading   # 상태 확인
sudo systemctl stop turtle-trading     # 중지
sudo systemctl restart turtle-trading  # 재시작
journalctl -u turtle-trading -f        # 로그 확인
```

### 11.4 방법 비교

| 방법 | 터미널 종료 후 | 서버 재부팅 후 | 난이도 | 권장 용도 |
|------|--------------|--------------|-------|---------|
| `docker compose up -d` | ✅ 유지 | ✅ 자동 시작 | 쉬움 | **프로덕션 (권장)** |
| tmux | ✅ 유지 | ❌ 수동 시작 | 쉬움 | 개발/테스트 |
| systemd | ✅ 유지 | ✅ 자동 시작 | 보통 | Docker 없이 프로덕션 |

> **권장**: Docker Compose를 사용하면 별도 설정 없이 24시간 실행됩니다.
